...
void initialize_order(unsigned long r, unsigned long n) {
  ...
  for (unsigned long i = 0; i < 6; ++i) {
    ...
    // Fill the rest of the order array with the remaining indices
    for (unsigned long i = 0; i < r; ++i) {
        for (unsigned long j = 0; j < r; ++j) {
            unsigned long index = i * r + j;
            if (index != corners[0] && index != corners[1] && index != corners[2] && index != corners[3] && index != corners[4] && index != corners[5]) {
                order[idx++] = index;
            }
        }
    }

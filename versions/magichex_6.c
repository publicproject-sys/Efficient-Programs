...
void initialize_order(unsigned long r, unsigned long n) {
  unsigned long idx = 0;
  ...
  for (unsigned long i = 0; i < 6; i++) {
    ...
    // Add the top and bottom rows (excluding corners)
    for (unsigned long i = corners[0] + 1; i < corners[1]; i++) {
        order[idx++] = i;
    }
    for (unsigned long i = corners[4] + 1; i < corners[5]; i++) {
        order[idx++] = i;
    }

    // Add outer elements of intermediate rows
    unsigned long row_start = r;
    for (unsigned long row = 2; row < r; row++) {
        if (row < (r + 1) / 2) {
            order[idx++] = row_start;
            unsigned long row_length = row < (r + 1) / 2 ? n + row - 1 : n + 2 * n - row - 1;
            order[idx++] = row_start + row_length - 1;
            row_start += row_length;
        }

        if (row > (r + 1) / 2) {
            row_start = r * (row - 1) + (row - 1) / 2;
            order[idx++] = row_start;
            order[idx++] = row_start + r - 2;
        }
    }

    // Fill in remaining inner values
    row_start = r;
    for (unsigned long row = 2; row < r; row++) {
        unsigned long row_length;
        if (row <= (r + 1) / 2) {
            row_length = n + row - 1;
            for (unsigned long i = row_start + 1; i < row_start + row_length - 1; i++) {
                order[idx++] = i;
            }
            row_start += row_length;
        } else {
            row_start = r * (row - 1) + (row - 1) / 2;
            row_length = n + 2 * n - row - 1;
            for (unsigned long i = row_start + 1; i < row_start + row_length - 1; i++) {
                order[idx++] = i;
            }
        }
    ...
    // Add outer elements of intermediate rows
    row_start = 0;
    for (unsigned long row = 0; row < r + 1; row++) {
        unsigned long row_length;
        if (row < (r + 1) / 2) {
            row_length = n + row - 1;
            for (unsigned long i = row_start + row_length + 1; i < r + row * r; i++) {
                order[idx++] = i;
            }
        } else if (row > (r + 1) / 2) {
            row_start = (row - 1) * r;
            unsigned long row_end = r * (row - 1) + (row - 1) / 2;

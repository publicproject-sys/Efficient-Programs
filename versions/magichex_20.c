...
void labeling(unsigned long n, long d, Var vs[], unsigned long index) {
    long i;
    unsigned long r = 2 * n - 1;
    unsigned long r_square = r * r;
    unsigned long mrvIndex = ULONG_MAX;
    long mrvRange = LONG_MAX;
    ...
    // Find the MRV variable
    for (unsigned long i = 0; i < r_square; i++) {
        if (vs[i].id >= 0 && vs[i].lo < vs[i].hi) {
            long currentRange = vs[i].hi - vs[i].lo;
            if (currentRange < mrvRange) {
                mrvRange = currentRange;
                mrvIndex = i;
            }
        }
    }

    // If no MRV variable is found (all variables are assigned), continue 
    if (mrvIndex == ULONG_MAX) {
        labeling(n, d, vs, index + 1);
        return;
    }
    ...
    for (i = vp->lo; i <= vp->hi; i++) {
        Var newvs[r_square];
        memcpy(newvs, vs, r_square * sizeof(Var));
        newvs[mrvIndex].lo = i;
        newvs[mrvIndex].hi = i;

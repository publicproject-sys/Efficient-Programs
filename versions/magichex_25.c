...
int sum(Var vs[], unsigned long nv, unsigned long stride, long sum, Var *vsstart, Var *vsend) {
 ...
 for (i = 0, vp = vs; i + 2 < nv; i += 3, vp += 3 * stride) {
    ...
    Var *vp_next = vp + stride;
    //assert(vp_next >= vsstart);
    //assert(vp_next < vsend);
    //assert(vp_next->id >= 0);
    hi -= vp_next->lo;
    lo -= vp_next->hi;

    // Third iteration of unrolling
    Var *vp_next_next = vp_next + stride;
    //assert(vp_next_next >= vsstart);
    //assert(vp_next_next < vsend);
    //assert(vp_next_next->id >= 0);
    hi -= vp_next_next->lo;
    lo -= vp_next_next->hi;
}

// Handle potential leftover iterations if nv is not a multiple of 3
for (; i < nv; i++, vp += stride) {
    //assert(vp >= vsstart);
    //assert(vp < vsend);
    //assert(vp->id >= 0);
    hi -= vp->lo;

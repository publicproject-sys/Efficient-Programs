...
unsigned long *eval_order = NULL; // Declare the global variable

void generate_eval_order(unsigned long *eval_order, unsigned long r) {
    unsigned long middle = (r + 1) / 2;

    for (unsigned long i = 0; i < r; i++) {
        if (i % 2 == 0) {
            eval_order[i] = middle - i / 2 - 1;
        } else {
            eval_order[i] = middle + i / 2;
        }
    }
}
...
int solve(unsigned long n, long d, Var vs[])
{
 ...
 restart:
  ...
  for (unsigned long i = 0; i < r; i++) {
    int f;
    unsigned long index = eval_order[i]; // Use the index from eval_order

    /* line */
    f = sum(vs + r * index + max(0, index + 1 - n), min(index + n, r + n - index - 1), 1, M, vs, vs + r * r);
    if (f == 0) return 0;
    if (f == 1) goto restart;

    /* column (diagonal down-left in the hexagon) */
    f = sum(vs + index + max(0, index + 1 - n) * r, min(index + n, r + n - index - 1), r, M, vs, vs + r * r);
    if (f == 0) return 0;
    if (f == 1) goto restart;

    /* diagonal (down-right) */
    f = sum(vs - n + 1 + index + max(0, n - index - 1) * (r + 1), min(index + n, r + n - index - 1), r + 1, M, vs, vs + r * r);
    if (f == 0) return 0;
    if (f == 1) goto restart;
}
  return 1;
...
int main(int argc, char *argv[])
{
  ...
  unsigned long r = 2 * n - 1;
  eval_order = (unsigned long *)malloc(r * sizeof(unsigned long));
  if (eval_order == NULL) {
    // Handle memory allocation failure
    exit(1);
  }
  generate_eval_order(eval_order, r);

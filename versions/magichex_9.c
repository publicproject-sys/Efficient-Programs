...
unsigned long calculate_ending(unsigned long n, unsigned long r, unsigned long row, unsigned long level) {
    return (row + 1) * r - (n - row) - level;
}

unsigned long calculate_begining(unsigned long n, unsigned long r, unsigned long row, unsigned long level) {
    return ((row - 1) * r) + (row - n) + level;
}

void calculate_all_edges(unsigned long n, unsigned long level, unsigned long r, unsigned long *corners_ends) {
    unsigned long idx = 0;
    for (unsigned long i = 0; i < r + 1; i++) {
        if (i >= level && i < r - level + 1) {
            if (i < (r + 1) / 2) {
                corners_ends[idx++] = i * r + level;
                corners_ends[idx++] = calculate_ending(n, r, i, level);
            } else if (i > (r + 1) / 2) {
                corners_ends[idx++] = calculate_begining(n, r, i, level);
...
void calculate_chain(unsigned long n, unsigned long r, unsigned long level, unsigned long *corners_ends, unsigned long *chain) {
    unsigned long idx = 0;
...
void initialize_order(unsigned long n) {
    unsigned long r = 2 * n - 1;
    order = (unsigned long *)malloc((r * r + 1) * sizeof(unsigned long));

    // Add the center of the hexagon first
    unsigned long idx = 0;
    order[idx++] = r * r / 2;

    for (long level = n - 2; level >= 0; level--) {
        unsigned long *corners_ends = (unsigned long *)malloc(2 * (r - 2 * level) * sizeof(unsigned long));
        calculate_all_edges(n, level, r, corners_ends);

        unsigned long chain_size = 2 * (r - 2 * level) + 2 * (n - 2 - level);
        unsigned long *chain = (unsigned long *)malloc(chain_size * sizeof(unsigned long));
        calculate_chain(n, r, level, corners_ends, chain);

        // Add the indices in reverse order for each layer
        for (long j = chain_size - 1; j >= 0; j--) {
            order[idx++] = chain[j];
        }
